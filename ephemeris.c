...
* / 向卫星发送星历以获得卫星钟差 ----------------------------- 
* 使用广播星历计算卫星钟差（GPS、Galileo、QZSS） 
* 参数 : gtime_t time I 卫星钟的时间（gpst） * eph_t eph I 广播星历 
* 返回值 : 不包含相对论修正的卫星钟差（秒） *
注意 : 参考文献[1]，[7]，[8] * 卫星钟差不包含相对论修正和tdg修正
*-----------------------------------------------------------------------------*/
extern double eph2clk(gtime_t time, const eph_t *eph)
{
    double t,ts;/*ts:初始的发射时刻与星历参考时刻的间隔(time不准导致ts不准),t:经卫星钟差修正后的间隔ts*/
    int i;
    
    trace(4,"eph2clk : time=%s sat=%2d\n",time_str(time,3),eph->sat);
    
    t=ts=timediff(time,eph->toc);/*初始的发射时刻(o)与星历参考时刻(n)的间隔*/
    
    for (i=0;i<2;i++) {/*经过两轮卫星钟差修正ts,求取最佳的时间间隔ts并存于t(迭代)*/
        t=ts-(eph->f0+eph->f1*t+eph->f2*t*t);/*dts=a0+a1*t+a2*t^2*/
    }
    trace(4,"ephclk: t=%.12f ts=%.12f dts=%.12f f0=%.12f f1=%.9f f2=%.9f\n",t,ts,
        eph->f0+eph->f1*t+eph->f2*t*t,eph->f0,eph->f1,eph->f2);

    return eph->f0+eph->f1*t+eph->f2*t*t;/*用求得的最佳时间间隔t求解卫星钟差*/
}
                    ...
                    double *var)
{
    ...
    tk=timediff(time,eph->toe);/*信号发射时刻到拟合轨道参数时刻的时间差*/
    
    switch ((sys=satsys(eph->sat,&prn))) {
        case SYS_GAL: mu=MU_GAL; omge=OMGE_GAL; break;
        case SYS_CMP: mu=MU_CMP; omge=OMGE_CMP; break;/*地球引力常数,地球自转角速度*/
        default:      mu=MU_GPS; omge=OMGE;     break;
    }
    M=eph->M0+(sqrt(mu/(eph->A*eph->A*eph->A))+eph->deln)*tk;/*平近点角*/
    
    for (n=0,E=M,Ek=0.0;fabs(E-Ek)>RTOL_KEPLER&&n<MAX_ITER_KEPLER;n++) {/*迭代解算偏近点角*/
    ...
    u=atan2(sqrt(1.0-eph->e*eph->e)*sinE,cosE-eph->e)+eph->omg;/*改正前的纬度参数*/
    r=eph->A*(1.0-eph->e*cosE);/*改正前的向径*/
    i=eph->i0+eph->idot*tk;/*改正前的倾角*/
    sin2u=sin(2.0*u); cos2u=cos(2.0*u);
    u+=eph->cus*sin2u+eph->cuc*cos2u;/*改正后的纬度参数*/
    r+=eph->crs*sin2u+eph->crc*cos2u;/*改正后的纬度向径*/
    i+=eph->cis*sin2u+eph->cic*cos2u;/*改正后的倾角*/
    x=r*cos(u); y=r*sin(u); cosi=cos(i);

    /* 对于beidou geo satellite单独的计算改正 */
    if (sys==SYS_CMP&&(prn<=5||prn>=59)) { /* ref [9] table 4-1 */
        O=eph->OMG0+eph->OMGd*tk-omge*eph->toes;/*改正升交点经度*/
        sinO=sin(O); cosO=cos(O);
        xg=x*cosO-y*cosi*sinO;/*卫星在地固坐标系中的坐标*/
        yg=x*sinO+y*cosi*cosO;
        zg=y*sin(i);
        sino=sin(omge*tk); coso=cos(omge*tk);
        rs[0]= xg*coso+yg*sino*COS_5+zg*sino*SIN_5;/*北斗IGSO,MEO与GPS坐标计算相同*/
        rs[1]=-xg*sino+yg*coso*COS_5+zg*coso*SIN_5;/*而在ECEF系统中,GEO卫星的轨道倾角为0*/
        rs[2]=-yg*SIN_5+zg*COS_5;/*因此升交点的赤经不能与近地点的自变量分开,因此在计算中引入了5度的旋转*/
    }
    else {
    ...
    tk=timediff(time,eph->toc);/*此处的time是经过之前多次钟差改正后求得的信号发射时的卫星钟面时刻最优解*/
    *dts=eph->f0+eph->f1*tk+eph->f2*tk*tk;/*求最优解时刻下的卫星钟差*/
    
    /* relativity correction 相对论改正*/
    *dts-=2.0*sqrt(mu*eph->A)*eph->e*sinE/SQR(CLIGHT);
    
    /* position and clock error variance */
    *var=var_uraeph(sys,eph->sva);/*定位与钟差方差*/
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    sys=satsys(sat,NULL);
    switch (sys) {/*根据卫星系统设置o,n文件允许的时间差阈值*/
    ...
    for (i=0;i<nav->n;i++) {/*遍历所有星历*/
        if (nav->eph[i].sat!=sat) continue;/*/筛选星历-相同prn*/
        if (iode>=0&&nav->eph[i].iode!=iode) continue;/*/筛选星历-相同iode;ephclk()调用本函数时传入的iode为-1,则无需检查iode*/
        if (sys==SYS_GAL) {
            ...
            /* this code is from 2.4.3 b34 but does not seem to be fully supported,
               ...
               这段代码来自于2.4.3 b34版本，但似乎并没有完全支持，所以目前我已经回退到b33版本的代码。*/
        ...
        if ((t=fabs(timediff(nav->eph[i].toe,time)))>tmax) continue;/*/筛选星历-n,o文件所设时间间隔阈值*/
        if (iode>=0) return nav->eph+i;/*/星历中的iode正常都>0,eph+i指向当前循环到的元素*/
        /*/iode<0时,除需满足低于阈值,还需要找出间隔最小的星历,用j记录位置*/
        if (t<=tmin) {j=i; tmin=t;} /* toe closest to time 更新离o文件最近的toe记录*/
    }
    if (iode>=0||j<0) {/*/iode都大于0,但时间间隔>阈值则给出匹配失败*/
        trace(2,"no broadcast ephemeris: %s sat=%2d iode=%3d\n",time_str(time,0),
              sat,iode);
        return NULL;
    }
    /*/至此,传入的iode必<0,返回的星历间隔小于阈值且时间距离最近*/
    trace(4,"seleph: sat=%d dt=%.0f\n",sat,tmin);
    return nav->eph+j;
    /*该函数实现的效果: 若传入的iode值>0,则根据iode值匹配星历,在导航数据中找同iode值的数据
                       若传入的iode值<0,则不仅要找时间间隔小于阈值的星历,且时间距离最小*/
}
                  ...
                  double *dts)
{
    ...
    sys=satsys(sat,NULL);/*/根据卫星号获悉所属导航系统*/
    
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        /*/匹配星历,传参iode=-1<0,会跳过idoe检查,返回最靠近o文件时间的星历*/
        if (!(eph=seleph(teph,sat,-1,nav))) return 0;/*/未匹配到时间间隔阈值内的星历数据会跳过*/
        /*/执行结束后星历匹配完成*/
        *dts=eph2clk(time,eph);/*/计算卫星钟差dts*/
    }
    else if (sys==SYS_GLO) {
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        if (!(eph=seleph(teph,sat,iode,nav))) return 0;/*/匹配星历,失败则跳*/
        eph2pos(time,eph,rs,dts,var);/*/计算卫星位置和钟差,存入rs和dts*/
        time=timeadd(time,tt);/*/人为添加时间增量,获得卫星的新位置和新钟差(通过扰动法计算出卫星的速度和钟漂,注意不是接受机)*/
        eph2pos(time,eph,rst,dtst,var);/*/计算卫星的新位置和新钟差,存入rst和dtst*/
        *svh=eph->svh;
    }
    else if (sys==SYS_GLO) {
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {/*/初始化*/
        for (j=0;j<6;j++) rs [j+i*6]=0.0;
        for (j=0;j<2;j++) dts[j+i*2]=0.0;
        var[i]=0.0; svh[i]=0;
        
        /* search any pseudorange 搜寻可用伪距 令pr等于obs.p(观测的伪距)*/
        ...
        /* transmission time by satellite clock 接收机t-传播用时(约伪距除以光速)≈卫星发射时刻的信号(gpst)*/
        time[i]=timeadd(obs[i].time,-pr/CLIGHT);/*由于未修改钟差且钟差数量级远大于传播用时,time[i]是一个较为粗略的卫星信号发射时刻的时间*/
        
        /* satellite clock bias by broadcast ephemeris 由广播星历计算卫星发射时刻的钟差
        ephclk 匹配合适的星历存于eph结构体中,eph中存有整个n文件所有的星历,且被包含在结构体nav中*/
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) {
            trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }
        time[i]=timeadd(time[i],-dt);/*/卫星钟差改正(钟面t-钟差＝信号发布时卫星钟差真值)*/

        /* satellite position and clock at transmission time 计算卫星发射信号时的位置速度和钟差*/
        if (!satpos(time[i],teph,obs[i].sat,ephopt,nav,rs+i*6,dts+i*2,var+i,
                    svh+i)) {/*/rs的每一列有6个元素,x y z;vx vy vz;dts每一列有两个元素分别为钟差和钟速*/
