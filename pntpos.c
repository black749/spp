                    ...
                    const double *azel, int ionoopt, double *ion, double *var)
{
    ...
    if (ionoopt==IONOOPT_BRDC||err==1) {
        *ion=ionmodel(time,nav->ion_gps,pos,azel);/*/计算电离层延迟*/
        *var=SQR(*ion*ERR_BRDCI);/*/方差*/
        return 1;
    }
    *ion=0.0;
                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        time=obs[i].time;
        sat=obs[i].sat;
        if (!(sys=satsys(sat,NULL))) continue;/*/验证卫星编号是否合理,及其所属的导航系统*/
        
        /* reject duplicated observation data */
        if (i<n-1&&i<MAXOBS-1&&sat==obs[i+1].sat) {/*/检测当前观测是否和下一个相邻数据重复*/
            trace(2,"duplicated obs data %s sat=%d\n",time_str(time,3),sat);
            i++;
            continue;
        }
        /* excluded satellite? 剔除有问题的卫星*/
        if (satexclude(sat,vare[i],svh[i],opt)) continue;
        
        /* geometric distance and elevation mask计算几何卫地距离与高度角屏蔽*/
        if ((r=geodist(rs+i*6,rr,e))<=0.0) continue;/*/r=卫地距,e=测站指向卫星方向的单位向量*/
        ...
        if (iter>0) {
            ...
            /* ionospheric correction 电离层改正*/
            if (!ionocorr(time,nav,sat,pos,azel+i*2,opt->ionoopt,&dion,&vion)) {/*/dion:gps L1*/
                continue;
            }
            if ((freq=sat2freq(sat,obs[i].code[0],nav))==0.0) continue;/*/获取当前载波频率,以计算当前载波的dion和方差*/
            dion*=SQR(FREQL1/freq);/*/由于是以L1 算得改正,若当前频率非GPS L1,则需乘以(L1频率/当前载波频率)的平方*/
            vion*=SQR(FREQL1/freq);
        
            /* tropospheric correction 对流层改正*/
            if (!tropcorr(time,nav,pos,azel+i*2,opt->tropopt,&dtrp,&vtrp)) {
                continue;
            }
        }
        /* psendorange with code bias correction 用码偏差校正伪距使用DCB*/
        if ((P=prange(obs+i,nav,opt,&vmeas))==0.0) continue;
        
        /* pseudorange residual */
        v[nv]=P-(r+dtr-CLIGHT*dts[i*2]+dion+dtrp);/*/伪距残差*/
        trace(4,"sat=%d: v=%.3f P=%.3f r=%.3f dtr=%.6f dts=%.6f dion=%.3f dtrp=%.3f\n",
            sat,v[nv],P,r,dtr,dts[i*2],dion,dtrp);
        
        /* design matrix */
        for (j=0;j<NX;j++) {
            H[j+nv*NX]=j<3?-e[j]:(j==3?1.0:0.0);/*/这里的H阵是转置得的?*/
        }
        ...
        vsat[i]=1; resp[i]=v[nv]; (*ns)++;/*/定位有效标志设为1,当前卫星伪距残差赋值,参与定位卫星个数++ */
        
        /* variance of pseudorange error */
        /*/计算伪距方差= 系统固有反差+卫星位置方差+钟差方差+伪距偏移方差+电离层对流层方差*/
                  ...
                  int *vsat, double *resp, char *msg)
{
    ...
    for (i=0;i<3;i++) x[i]=sol->rr[i];/*/首历元为空,x=0;非首历元的话,取上一个历元的结果作为初值*/

    for (i=0;i<MAXITR;i++) {
        /*/计算伪距残差v(&方差var)+构建H阵;v=Hx+e x阵:xyz+dtr v阵:加了各种改正项的伪距-卫地距*/
        ...
        /* weight by variance (lsq uses sqrt of weight) 方差定权*/
        for (j=0;j<nv;j++) {
            sig=sqrt(var[j]);
            v[j]/=sig;
            for (k=0;k<NX;k++) H[k+j*NX]/=sig;
        }
        /* least square estimation 最小二乘*/
        ...
        if (norm(dx,NX)<1E-4) {/*/当改正数足够小时储存结果,结束迭代*/
            /*/将结果存入结构体sol*/
            sol->type=0;
...
* return : status(1:ok,0:error)
* 使用伪距和多普勒观测值进行单点定位，计算接收机的位置、速度和钟差 
* 参数 : obsd_t obs 输入，观测数据 
* int n 输入，观测数据的数量 * nav_t nav 输入，导航数据 * prcopt_t opt 输入，处理选项 
* sol_t sol 输入/输出，解算结果 * double azel 输入/输出，方位角/仰角（弧度）（NULL: 不输出） 
* ssat_t ssat 输入/输出，卫星状态（NULL: 不输出） * char msg 输出，错误信息 
* 返回值 : 状态 (1: 成功, 0: 错误) 
*-----------------------------------------------------------------------------*/
                  ...
                  char *msg)
{
    ...
    sol->time=obs[0].time;/*/输出时间设为接收机时间*/
    msg[0]='\0';/*/给字符串加结尾*/
    ...
    if (ssat) {/*/这是干啥的?*/
        for (i=0;i<MAXSAT;i++) {
            ssat[i].snr_rover[0]=0;/*/移动站信号强度*/
            ssat[i].snr_base[0]=0;/*/基站信号强度*/
        }
        for (i=0;i<n;i++)
            ssat[obs[i].sat-1].snr_rover[0]=obs[i].SNR[0];
    }
    
    if (opt_.mode!=PMODE_SINGLE) { /* for precise positioning ,spp时不运行*/
        opt_.ionoopt=IONOOPT_BRDC;/*/电离层改正采用广播星历*/
        /*/北斗ppp的spp必须用广播星历*/
        opt_.tropopt=TROPOPT_SAAS;/*/对流层改正采用saastamoinen*/
    }
    /* satellite positons, velocities and clocks 计算卫星的位置和速度,钟差和钟速*/
    satposs(sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh);
    
    /* estimate receiver position and time with pseudorange 使用伪距测量估计接收机的位置和时间*/
    ...
    /* estimate receiver velocity with Doppler 使用多普勒效应估计接收机的速度*/
    if (stat) {
