...
static obs_t obss={0};          /* observation data 只存一个历元之下的数据*/
static nav_t navs={0};          /* navigation data 存整个文件中的数据*/
static sbs_t sbss={0};          /* sbas messages */
static sta_t stas[MAXRCV];      /* station infomation 站点信息*/
...
/* show message and check break 显示信息并检查用户是否终止----------------------------------------------*/
...
/* output reference position 输出基站信息-------------------------------------------------------------*/
...
/* search next observation data index 搜寻下一组观测数据的索引----------------------------------------*/
...
/* update rtcm ssr correction 更新RTCM SSR校正------------------------------------------------------*/
...
/* input obs data, navigation messages and sbas correction 输入观测数据、导航消息和SBAS校正------------*/
static int inputobs(obsd_t *obs, int solq, const prcopt_t *popt)
{
    ...
    if (!revs) { /* input forward data */
        ...
        if (popt->intpref) {
        	/* interpolate nearest timestamps 插值最近的时间戳*/
...
/* output to file message of invalid time mark 将无效时间标记的消息输出到文件--------------------------*/
...
/* fill structure sol_t for time mark 为时间标记填充sol_t结构体---------------------------------------*/
static sol_t fillsoltm(const sol_t solold, const sol_t solnew, const gtime_t tm)
{
    ...
    /* interpolation position and speed of time mark 对时间标记进行位置和速度的插值*/
    t1 = solold.time;
    t2 = solnew.time;
    sol.time = tm;
...
/* carrier-phase bias correction by ssr通过SSR进行载波相位偏差校正 -----------------------------------*/
                    ...
                    rtk_t *rtk, int mode)
{
    ...
    /* initialize unless running backwards on a combined run with phase reset disabled 在禁用相位重置的组合运行中，除非反向运行，则进行初始化。*/
    if (mode==0 || !revs || popt->soltype==2)
        rtkinit(rtk,popt);/*/rtk结构体变量初始化*/
    
    rtcm_path[0]='\0';
    
    while ((nobs=inputobs(obs_ptr,rtk->sol.stat,popt))>=0) {/*/读o文件一个历元下所有卫星的数据(所有系统的卫星都读)*/
        /*/inputobs函数用于将全局变量obss中的数据写入局部变量obs*/
        /* exclude satellites 排除卫星*/
        for (i=n=0;i<nobs;i++) {/*/根据设置的卫星系统和废弃的卫星编号剔除*/
            if ((satsys(obs_ptr[i].sat,NULL)&popt->navsys)&&/*/比如本历元观测54个卫星,剔除后,按顺序赋值obs[i]*/
                popt->exsats[obs_ptr[i].sat-1]!=1) obs_ptr[n++]= obs_ptr[i];
        }
        if (n<=0) continue;/*/本历元观测了多少有用颗卫星*/
        
        /* carrier-phase bias correction 载波相位偏差矫正*/
    ...
    free(obs_ptr); /* moved from stack to heap to kill a stack overflow warning 将数据从堆栈（stack）移动到堆（heap），以避免堆栈溢出警告*/
}
/* validation of combined solutions 对组合解进行验证-------------------------------------------------*/
...
/* combine forward/backward solutions and save results 组合前方和后方结果-----------------------------*/
    ...
    for (i=0,j=isolb-1;i<isolf&&j>=0;i++,j--) {
        if ((tt=timediff(solf[i].time,solb[j].time))<-DTTOL) {
            ...
            if ((popt->mode==PMODE_KINEMA||popt->mode==PMODE_MOVEB)&&
                ...
                /* degrade fix to float if validation failed 如果验证失败，将修正（fix）的结果降级为浮点数（float）*/
...
/* read prec ephemeris, sbas data, tec grid and open rtcm 读先验星历、SBAS,tec等数据 ----------------*/
                        ...
                        nav_t *nav, sbs_t *sbs)
{
    ...
    /* read precise ephemeris files 读精密星历*/
    for (i=0;i<n;i++) {
        if (strstr(infile[i],"%r")||strstr(infile[i],"%b")) continue;
        readsp3(infile[i],nav,0);
    }
    /* read precise clock files读精密钟差 */
    for (i=0;i<n;i++) {
        if (strstr(infile[i],"%r")||strstr(infile[i],"%b")) continue;
        readrnxc(infile[i],nav);
    }
    /* read sbas message files 读sbas数据*/
    for (i=0;i<n;i++) {
        if (strstr(infile[i],"%r")||strstr(infile[i],"%b")) continue;
        sbsreadmsg(infile[i],prcopt->sbassatsel,sbs);
    }
    /* allocate sbas ephemeris 为SBAS星历分配空间*/
...
/* free prec ephemeris and sbas data 释放各星历所占空间-----------------------------------------------*/
...
/* read obs and nav data 读obs与nav数据-------------------------------------------------------------*/
static int readobsnav(gtime_t ts, gtime_t te, double ti, char **infile,
                      ...
                      obs_t *obs, nav_t *nav, sta_t *sta)
 {
    ...
    for (i=0;i<n;i++) {
        ...
        /* read rinex obs and nav file 从文件指针处读文件?*/
    ...
    /* sort observation data 分类会观测数据*/
    nepoch=sortobs(obs);
    
    /* delete duplicated ephemeris 删除重复的星历数据 */
    uniqnav(nav);

    /* set time span for progress display 设置进度显示的时间跨度 */
...
/* average of single position 单点定位的平均值------------------------------------------------------*/
...
/* station position from file 从文件获取站点位置----------------------------------------------------*/
static int getstapos(const char *file, char *name, double *r)
{
    FILE *fp;
    char buff[256],sname[256],*p,*q,aim[20];
    double pos[3];

    trace(3,"getstapos: file=%s name=%s\n",file,name);
    
    size_t len = strlen(file);
    const char* lastThree = file + len - 3;
    /*printf("字符串末尾的三个字符为: %s\n", lastThree);*/
    if (strstr(lastThree, "snx"))
    {
        sprintf(aim, "STAX   %s", name);
        if (!(fp = fopen(file, "r"))) {
        ...
        while (fgets(buff, sizeof(buff), fp))
        {
            if(strstr(buff,"+SOLUTION/ESTIMATE\n"))
            {
                while (fgets(buff, sizeof(buff), fp))
                {
                    if (strstr(buff + 7, aim))
                    {
                        int i = 0;
                        do {
                            r[i] = str2num(buff, 47, 21);
    ...
    else if (strstr(lastThree, "crd")) 
    {
        ...
        while (fgets(buff, sizeof(buff), fp))
        {
            if (strstr(buff, "%%"))
            {
                while (fgets(buff, sizeof(buff), fp))
                {
                    if (strstr(buff + 4, aim))
                    {
                        r[0] = str2num(buff, 12, 12);
                        r[1] = str2num(buff, 28, 12);
                        r[2] = str2num(buff, 44, 12);
                        fclose(fp);
                        return 1;
                    }
                }
...
/* antenna phase center position 天线相位中心位置 基于选项计算接收机位置------------------------------*/
...
/* close procssing session 关闭处理会话-----------------------------------------------------------*/
...
/* read ocean tide loading parameters 读取海洋潮汐加载参数-----------------------------------------*/
...
/* write header to output file 向输出文件中写入头部信息---------------------------------------------*/
...
/* open output file for append 打开输出文件以进行追加写入操作---------------------------------------*/
...
static void namefiletm(char *outfiletm, const char *outfile)
{
    ...
    /* if no file extension, then name time marks file as name of outfile + _events.pos如果没有文件扩展名，则将时间标记文件命名为输出文件名 + _events.pos。 */
    if (i == 0) {
...
/* execute processing session 执行处理会话      ------------------------------------------------*/
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    /* read ionosphere data file 读取电离层数据文件*/
    ...
    /* read erp data 读取地球自转参数（Earth Rotation Parameters）数据文件*/
    if (*fopt->eop) {
    ...
    /* read obs and nav data 读取obs与nav o文件存于obss,n文件存于navs*/
    ...
    /* read dcb parameters 读取差分码偏差（Differential Code Bias）参数*/
    if (*fopt->dcb) {
    ...
    /* rover/reference fixed position 流动站/参考站的固定位置*/
    /*/ fopt->stapos 测站位置文件路径 */
    ...
    if (popt_.mode == PMODE_SINGLE)
    {
        ...
        int i;
        for (i = 0; i < 4; i++) {
            name[i] = toupper(stas[0].name[i]);
        }
        name[i] = '\0';
        if (!getstapos(fopt->stapos, name, rtk_ptr->rb))
        {
            int startPos = strlen(fopt->stapos) - 11;
            strncpy(&fopt->stapos[startPos], "true_crd.true_crd", 17);
            if (!getstapos(fopt->stapos, name, rtk_ptr->rb)) {
                trace(1, "all no station position: %s\n", name);
            }

        }
    }

    /* open solution statistics 打开处理后的统计数据*/
    ...
    /* name time events file 命名时间事件文件*/
    /*namefiletm(outfiletm, outfile);*/
    /* write header to file with time marks 在带有时间标记的文件中写入标题*/
    ...
    if (popt_.mode==PMODE_SINGLE||popt_.soltype==0) {
        if ((fp=openfile(outfile)) && (fptm=openfile(outfiletm))) {
            procpos(fp,fptm,&popt_,sopt,rtk_ptr,0); /* forward 前向*/
            fclose(fp);
            fclose(fptm);
        }
    }
        ...
        if ((fp=openfile(outfile)) && (fptm=openfile(outfiletm))) {
            ...
            procpos(fp,fptm,&popt_,sopt,rtk_ptr,0); /* backward 后向*/
            fclose(fp);
            fclose(fptm);
        }
    }
        ...
        if (solf&&solb) {
            isolf=isolb=0;
            procpos(NULL,NULL,&popt_,sopt,rtk_ptr,1); /* forward前向 */
            revs=1; iobsu=iobsr=obss.n-1; isbs=sbss.n-1;
            procpos(NULL,NULL,&popt_,sopt,rtk_ptr,1); /* backward 后向*/
            
            /* combine forward/backward solutions 合并正向/反向解算结果*/
    ...
    /* free rtk, obs and nav data 释放数据*/
    rtkfree(rtk_ptr);
    free(rtk_ptr);
    freeobsnav(&obss,&navs);
...
/* execute processing session for each rover 为每个流动站执行处理会话---------------------------------*/
                     ...
                     const char *rov)
{
    ...
    if (i<n) { /* include rover keywords 多个流动站*/
        ...
        /* execute processing session 处理单个流动站*/
        stat=execses(ts,te,ti,popt,sopt,fopt,flag,infile,index,n,outfile);
    }
    return stat;
}
/* execute processing session for each base station 循环每个基准站执行会话--------------------------*/
                     ...
                     const char *rov, const char *base)
{
    ...
    /* read prec ephemeris and sbas data 读取精密星历和SBAS数据  读取SP3与pclk存于navs中*/
    readpreceph(infile,n,popt,&navs,&sbss);
    
    for (i=0;i<n;i++) if (strstr(infile[i],"%b")) break;
    
    if (i<n) { /* include base station keywords   基准站读入 */
...
/* post-processing positioning 后处理定位 -------------------------------------------------
* post-processing positioning
* args   : gtime_t ts       I   processing start time (ts.time==0: no limit) 参数 : gtime_t ts I 处理开始时间 (ts.time==0: 无限制) 
*        : gtime_t te       I   processing end time   (te.time==0: no limit)      gtime_t te I 处理结束时间 (te.time==0: 无限制) *
*          double ti        I   processing interval  (s) (0:all)处理间隔 (s) (0:全部) 
*          double tu        I   processing unit time (s) (0:all)处理单元时间 (s) (0:全部) 
*          prcopt_t *popt   I   processing options  处理选项
*          solopt_t *sopt   I   solution options    解算选项
*          filopt_t *fopt   I   file options        文件选项
*          char   **infile  I   input files (see below) 输入文件(下方说明)
*          int    n         I   number of input files   输入文件数量
*          char   *outfile  I   output file ("":stdout, see below)  输出文件
*          char   *rov      I   rover id list        (separated by " ") 流动站编号
*          char   *base     I   base station id list (separated by " ") 基准站编号
* return : status (0:ok,0>:error,1:aborted)  返回值:状态 (0:成功,0>:错误,1:中止)
...
*          data.
* 注意 : 输入文件应包含观测数据, 导航数据, 精密星历/钟差 (可选), sbas日志文件 (可选), 
         ssr报文日志文件 (可选) 和tec网格文件 (可选). 输入文件列表中的第一个观测数据文件 
*        被识别为流动站数据。 
*
...
*         根据文件扩展名识别输入文件类型如下: * .sp3,.SP3,.eph,.EPH: 精密星历 (sp3c) 
*           .sbs,.SBS,.ems,.EMS : sbas消息日志文件 (rtklib或ems) 
*           .rtcm3,.RTCM3 : ssr报文日志文件 (rtcm3) 
*           .i,.I : tec网格文件 (ionex) 
             其他 : rinex观测数据, 导航文件, gnav文件, hnav文件, qnav文件或钟差文件 
*
*          inputs files can include wild-cards (*). if an file includes
*          wild-cards, the wild-card expanded multiple files are used.
*           输入文件可以包含通配符 (). 如果文件包含通配符, 则会使用通配符扩展成多个文件。
*
...
*          keywords.
*           输入文件可以包含关键字. 如果文件包含关键字, 则关键字会根据日期, 时间, 流动站编号和基准站编号进行替换, 并运行多个会话分析.
*           关键字的替换规则请参考reppath()函数。
...
*          输出文件也可以包含关键字. 如果输出文件不包含关键字, 则多个会话分析的结果会输出到一个单一的输出文件中。
