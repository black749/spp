...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if (opt->refpos<=POSOPT_RINEX&&opt->mode!=PMODE_SINGLE&&
        ...
        for (i=0;i<6;i++) rtk->rb[i]=i<3?opt->rb[i]:0.0;/*/全部初始化为0*/
    }
    /* count rover/base station observations */
    for (nu=0;nu   <n&&obs[nu   ].rcv==1;nu++) ;/*/确定流动站的观测卫星数目前只确定了G*/
    for (nr=0;nu+nr<n&&obs[nu+nr].rcv==2;nr++) ;/*/确定基准站的观测卫星数*/

    time=rtk->sol.time; /* previous epoch 初始为0*/
    
    /* rover position and time by single point positioning 单点定位的移动站位置和时间*/
    if (!pntpos(obs,nu,nav,&rtk->opt,&rtk->sol,NULL,rtk->ssat,msg)) {
        errmsg(rtk,"point pos error (%s)\n",msg);
        /*/此步结束后rtk结构体中的部分数据已经得到输出,如rr,qr,qv*/
        if (!rtk->opt.dynamics) {
            outsolstat(rtk,nav);
            return 0;
        }
    ...
    /*if (rtk->sol.rr[0] && rtk->sol.rr[1] && rtk->sol.rr[2])
    //{
    //    printf("%s B-%12.6f L-%12.6f H=%12.6f \n", time_str(rtk->sol.time, 6), rtk->sol.rr[0], rtk->sol.rr[1], rtk->sol.rr[2]);
    //}
  /*  if (1)
    {
        printf("%s  \n", time_str(rtk->sol.time, 6));
    }*/

    /* return to static start if long delay without rover data 如果长时间没有接收到移动站数据，返回静态起点*/
