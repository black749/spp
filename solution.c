    ...
    const solopt_t* opt)
{
    ...
    trace(3, "outpos  :\n");
    //将xyz转化为BLH
    ecef2pos(sol->rr, pos);//这里的pos是弧度制下的参数,可以转为经纬度
    //将输出解置于xyz协方差阵
    soltocov(sol, P);
    //转化为enu的协方差
    covenu(pos, P, Q);
    //根据输出选项中的height字段的值，调整位置中的高度。
    //如果height为1，表示输出大地高度，需要从位置中减去地球椭球体高程。
    ...
    if (opt->degf) {
        ...
        //输出 latitude(deg) longitude(deg)
        p += sprintf(p, "%s%s%14.9f%s%14.9f", s, sep, pos[0] * R2D, sep, pos[1] * R2D);
    }
    //拼接输出height(m)   Q  ns   sdn(m)   sde(m)   sdu(m)  sdne(m)  sdeu(m)  sdun(m) age(s)  ratio
    p += sprintf(p, "%s%10.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f"
        "%s%6.2f%s%6.1f",
        sep, pos[2], sep, sol->stat, sep, sol->ns,
        sep, SQRT(Q[4]), sep,
...
static int outpos(uint8_t *buff, const char *s, const sol_t *sol, const double* rb,
                  const solopt_t *opt)
{
    double pos[3], pos1[3], vel[3], enu[3], rr[3], dms1[3], dms2[3], P[9], Q[9];
    const char *sep=opt2sep(opt);
    char *p=(char *)buff;
    int i;
    ...
    /*/pos指基站坐标位置BLH形式*/
    ecef2pos(rb, pos1);
    /*/rr指计算所得位置减去基站位置得到的向量*/
    ecef2enu(pos1,rr,enu);
    /*/将输出解置于xyz协方差阵*/
    soltocov(sol,P);
    /*/转化为enu的协方差*/
    covenu(pos,P,Q);
    ...
    /*/拼接输出height(m)  Q  ns        de(m)   dn(m)   du(m) sde(m)  sdn(m)  sdu(m) age(s)  ratio*/
    /*/                xyz   dx dy dz*/
    p+=sprintf(p,"%s%10.4f%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f"
               "%s%6.2f%s%6.1f",
               sep,pos[2], sep,
        /*/xyz*/
        sol->rr[0], sep, sol->rr[1], sep, sol->rr[2],
                sep,sol->stat,sep,sol->ns, sep,
        /*/dxdydz*/
         SQRT(sol->qr[0]), sep, SQRT(sol->qr[1]), sep,SQRT(sol->qr[2]),
        /*/e n u*/
                sep, enu[0], sep, enu[1], sep, enu[2]
                ,sep,sqvar(Q[0]),sep,sqvar(Q[4]),
               sep,sqvar(Q[8]),sep,sol->age,sep,sol->ratio);
    ...
    if (opt->posf==SOLF_LLH) { /* lat/lon/hgt */
        if (opt->degf) {
            p+=sprintf(p,"%16s%s%16s%s%10s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s"
                       ...
                       "Q",sep,"ns",sep,"sdn(m)",sep,"sde(m)",sep,"sdu(m)",
                       sep,"sdne(m)",sep,"sdeu(m)",sep,"sdue(m)",sep,"age(s)",
                       sep,"ratio");
        }
        else {
            p+=sprintf(p,"%14s%s%14s%s%10s%s%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s"
                       "%8s%s%6s%s%6s",
                       "latitude(deg)",sep,"longitude(deg)",sep,"height(m)",sep,
                        "ecef-X", sep, "ecef-Y", sep, "ecef-Z", sep,
                        "Q", sep, "ns", sep,
                        "dx", sep, "dy", sep, "dz", sep,
                       "de(m)",sep,"dn(m)",sep,"du(m)",sep,
                       "sde(m)",sep,"sdn(m)",sep,"sdu(m)",sep," age(s)",sep,
                       " ratio");
        }
        if (opt->outvel) {
            p+=sprintf(p,"%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s",
                   ...
                   const solopt_t *opt)
{
    ...
    /*/如果解决方案的状态（sol->stat）小于或等于SOLQ_NONE或者选项opt->posf等于SOLF_ENU并且基站位置的范数（norm(rb,3)）小于等于0，则返回0。*/
    ...
    switch (opt->posf) {
        ...
        case SOLF_LLH:  p += outpos(p, s, sol, rb, opt);   break;
...
* args   : FILE   *fp       I   output file pointer 输出文件指针
*          sol_t  *sol      I   solution 储存输出内容的结构体
*          double *rb       I   base station position {x,y,z} (ecef) (m)
*          solopt_t *opt    I   solution options输出选项
* return : none
*-----------------------------------------------------------------------------*/
