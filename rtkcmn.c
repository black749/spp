...
const prcopt_t prcopt_default={ /* defaults processing options */
    PMODE_KINEMA,0,2,SYS_GPS|SYS_GLO|SYS_GAL,   /* mode,soltype,nf,navsys 定位模式,解类型,频率数量,导航系统*/
    15.0*D2R,{{0,0}},           /* elmin,snrmask 最小可见高度角,信噪比掩码*/
    0,3,3,1,0,1,                /* sateph,modear,glomodear,gpsmodear,bdsmodear,arfilter 
                                星历类型,AR模式,GLONASS AR模式。GPS AR模式。BDS AR模式。 AR滤波模式。*/
    20,0,4,5,10,20,             /* maxout,minlock,minfixsats,minholdsats,mindropsats,minfix
                                最大卫星失锁时间,最小连续锁定时间,最小固定卫星数量,最小保持卫星数量,最小丢失卫星数量,最小固定解数量*/ 
    1,1,1,1,0,                  /* armaxiter,estion,esttrop,dynamics,tidecorr 
                                最大AR迭代次数,IONOSPHERE估计开关,TROPOSPHERE估计开关,动力学模型,潮汐修正开关*/                       
    1,0,0,0,0,                  /* niter,codesmooth,intpref,sbascorr,sbassatsel 
                                  定位迭代次数,码平滑开关,无整数模糊度开关,SBAS修正开关,SBAS选择卫星开关*/                    
    0,0,                        /* rovpos,测站位置.refpos 参考站位置*/
    {300.0,300.0,300.0},        /* eratio[] 观测误差比例因子*/
    {100.0,0.003,0.003,0.0,1.0,52.0,0.0,0.0}, /* err[-,base,el,bl,dop,snr_max,snr,rcverr] 观测误差参数*/
    {30.0,0.03,0.3},            /* std[] 测量噪声参数*/
    {1E-4,1E-3,1E-4,1E-1,1E-2,0.0}, /* prn[] 卫星噪声参数*/
    5E-12,                      /* sclkstab卫星钟稳定度*/
    {3.0,0.25,0.0,1E-9,1E-5,3.0,3.0,0.0}, /* thresar 残差阈值*/
	0.0,0.0,0.05,0,             /* elmaskar,elmaskhold,thresslip,thresdop, 
                                  AR解的最小高度角,保持解的最小高度角,失锁阈值,多普勒阈值*/
	0.1,0.01,30.0,              /* varholdamb,gainholdamb,maxtdif 
                                整数模糊度方差的初始值,滤波器增益系数,最大站间差分时间差*/
    {5.0,30.0},                 /* maxinno {phase,code} ,最大测量残差*/
    {0},{0},{0},                /* baseline,ru,rb,基线长度,接收机使用的天线相关参数,卫星之间的相对偏差*/
    {"",""},                    /* anttype 天线类型*/
    {{0}},{{0}},{0},            /* antdel,pcv,exsats 接收机天线延迟,天线PCV文件,不用于定位解的卫星*/
    1,1                         /* maxaveep,initrst 平均动方差估计的最大时长,初始解的模式*/
};
const solopt_t solopt_default={ /* defaults solution output options */
    SOLF_LLH,TIMES_GPST,1,3,    /* posf,times,timef,timeu 
    输出位置格式，默认为SOLF_LLH，表示经纬度高程.输出时间格式，默认为TIMES_GPST，表示卫星导航系统时间。输出时间字段，默认为1，表示年月日时分秒.输出时间单位，默认为3，表示秒*/
    0,0,0,0,0,0,0,              /* degf,outhead,outopt,outvel,datum,height,geoid 
                                 输出角度格式,输出标题行开关,输出选项,输出速度开关,大地基准改正开关,输出高程开关,输出大地高开关*/
    0,0,0,                      /* solstatic,sstat,trace静态定位解开关 单点定位统计信息开关 输出调试信息开关*/
    {0.0,0.0},                  /* nmeaintv NMEA输出间隔时间*/
    " ",""                      /* separator/program name 分隔符默认空格,程序名称默认空字符串*/
};
...
/* multiply matrix 矩阵乘法,NN表示都不转置,NT表示第一个矩阵转第二个不转,以此类推-*/
/*效果为 返回C=alpha*A*B+beta*C 参数 n:第一个矩阵的行 m:第一个矩阵的列或第二个矩阵的行 k:第二个矩阵的列*/
extern void matmul(const char *tr, int n, int k, int m, double alpha,
                   const double *A, const double *B, double beta, double *C)
{
    double d;
               ...
               double *Q)
{
    ...
    if (m<n) return -1;
    Ay=mat(n,1);
    matmul("NN",n,1,m,1.0,A,y,0.0,Ay); /* Ay=A*y (BPL)*/
    matmul("NT",n,n,m,1.0,A,A,0.0,Q);  /* Q=A*A' (B^τ PB)*/
...
* 将ECEF（地心地固坐标系）位置转换为大地坐标系位置。

函数参数： - double r: 输入参数，ECEF位置坐标{x，y，z}（单位：米） - double pos: 输出参数，大地坐标系位置{纬度，经度，高程}（单位：弧度，米）

返回值： - 无

注： - 使用WGS84椭球模型，计算的高程为椭球面到点的高度。
*-----------------------------------------------------------------------------*/
...
* notes  : matirix stored by column-major order (fortran convention)
* 计算ECEF到局部坐标转换矩阵 
* 参数 : double pos I 大地坐标 {纬度,经度} (弧度) 
* double E O ECEF到局部坐标转换矩阵 (3x3) 
* 返回值 : 无 * 注意 : 矩阵按列主序（Fortran约定）存储
*-----------------------------------------------------------------------------*/
...
* return : none
* 将ECEF（地心地固坐标系）协方差矩阵转换为局部切平面坐标系。

函数参数： - double pos: 输入参数，大地坐标系位置{纬度，经度}（单位：弧度） 
- double P: 输入参数，ECEF坐标系中的协方差矩阵 - double *Q: 输出参数，局部切平面坐标系中的协方差矩阵
...
extern void covenu(const double *pos, const double *P, double *Q)
{
    ...
    /*根据给定的参数，这段代码实现了对大小为3x3的矩阵EP和矩阵P进行普通矩阵相乘的操作，将结果存储在矩阵Q中。
    //参数NT表示NT表示第一个矩阵转第二个不转
    //参数alpha设为1.0，代表不对结果进行缩放。
    //参数beta设为0.0，表示矩阵C中原有的累加乘积的系数为0，只存储新计算的乘积结果。*/
